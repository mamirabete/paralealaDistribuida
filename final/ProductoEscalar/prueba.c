...
int main(int argc, char *argv[]) {
  ...
  int elementosPorProceso = cantidadElementos / size;
  int elementosRestantes = cantidadElementos % size;
  ...
  par = 0; 
  impar = 0;
  for (count = rank * elementosPorProceso; count < (rank + 1) * elementosPorProceso; ++count) {
  ...
  if (rank == 0 && elementosRestantes > 0) {
  ...
  if (rank == 0) {
    if (outCsv != 1){
      ...
      printf("%d,%d,%lli,%f,%f\n", cantidadElementos, size, productoEscalar, tiempo, tiempoSeg);
    }
  }
